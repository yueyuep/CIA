...
import graph.Base.ParseUtil;
import graph.Unity.Graph;
...
public class AST2Graph extends ParseUtil implements Graph {
    ...
    public static AST2Graph newInstance(String srcFilePath) {
        try {
    ...
    public void renameNetworkVar() {
        ...
        for (Object node : mNetwork.nodes()) {
            if (node instanceof RangeNode) {
                nodes.add(((RangeNode) node).getmNode());
            }
        }
    ...
    public <T extends Node> boolean travelNode(T nodeRoot) {
        ...
        for (Node node : nodes) {
            ...
            String nodeClass = Utils.getClassLastName(nodeClassPackage);
            node.removeComment();
...
//                logInfo(value);
    ...
    public void printEndpointPair(EndpointPair endpointPair) {
        logInfo(endpointPair.nodeU().getClass() + ": " + ((RangeNode) endpointPair.nodeU()).getmNode().toString()
                + ": " + ((RangeNode) endpointPair.nodeU()).getmNode().getBegin().get().line);
        logInfo(endpointPair.nodeV().getClass() + ": " + ((RangeNode) endpointPair.nodeV()).getmNode().toString()
