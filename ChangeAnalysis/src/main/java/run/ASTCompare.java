...
public class ASTCompare {
    ...
    private void procesDelete() {
        ...
        for (String del : delFile) {
            try {
                MethodProcess methodProcess_old = (MethodProcess) parse(fileMap.getSourcedir() + del, new MethodProcess());
    ...
    private void processAdd() {
        ...
        for (String add : addFile) {
            MethodProcess methodProcess_new = (MethodProcess) parse(fileMap.getTargetdir() + add, new MethodProcess());
    ...
    private void singleMethodCompare(HashMap<CtMethod, List<CtVariable>> ctMethodListHashMap) {
        for (CtMethod ctMethod : ctMethodListHashMap.keySet()) {
            ...
            variables.forEach(ctVariable -> {
                /*todo:ctVariable.getPosition()==null? */
                if (ctVariable.getPosition().isValidPosition()) {
